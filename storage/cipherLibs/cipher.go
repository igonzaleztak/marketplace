...
// RecoverDataFromSignature recovers the public key of the signature and its Ethereum Address
func RecoverDataFromSignature(hash, signature []byte) (common.Address, []byte, error) {
	...
	hashKey := sha3.NewLegacyKeccak256()
	hashKey.Write(pubKey[1:])
	aux := hashKey.Sum(nil)
	addr := aux[len(aux)-20:]

	return common.BytesToAddress(addr), pubKey, nil
}
